...
{
    /**
     * Constant for check right cache id
     */
    const CHECKRIGHT = 'checkRight';
    ...
    /**
     * storing recurrent data into class
     *
     * @var array
     */
    ...
    {
        $isCacheable = (Tinebase_Core::get(Tinebase_Core::CUSTOMEXPIRABLECACHE)
                && (Tinebase_Core::get(Tinebase_Core::SERVER_CLASS_NAME) == 'ActiveSync_Server_Http'));

        if ($isCacheable) {
            $params = array(__FUNCTION__, $_application, $_accountId, $_right);
            $cacheId = Tinebase_Helper::arrayToCacheId($params);
        ...
        $classCacheId = Tinebase_Helper::convertClassCacheId(implode('', $roleMemberships));
        ...
        if ($isCacheable) {
            $cache->save($result, $cacheId, array('AShasRight', $_accountId));
        }
    ...
    public function getRoleMemberships($accountId, $type = Tinebase_Acl_Rights::ACCOUNT_TYPE_USER)
    {
        if ($type === Tinebase_Acl_Rights::ACCOUNT_TYPE_USER) {
            ...
            $classCacheId = Tinebase_Helper::convertClassCacheId($accountId . implode('', $groupMemberships) . $type);
        } else if ($type === Tinebase_Acl_Rights::ACCOUNT_TYPE_GROUP) {
            $accountId = Tinebase_Model_Group::convertGroupIdToInt($accountId);

            $classCacheId = Tinebase_Helper::convertClassCacheId($accountId . $type);
        } else {
    ...
    /**
     * Create check right cache id
     * @param integer $userId
     * @param string $right
     * @param string $appName
     * @return string
     */
    public static function createCheckRightCacheId($userId, $right, $appName)
    {
        return Tinebase_Helper::arrayToCacheId(array(self::CHECKRIGHT, $userId, $right, $appName));
    }
    ...
    protected function _invalidateRightsCache($roleId, $roleRights)
    {
        ...
        foreach ($roleRights as $right) {
            $rightData['right']   = strtoupper($right['right']);
            $rightData['appName'] = Tinebase_Application::getInstance()->getApplicationById($right['application_id'])->name;
            $rightsInvalidateCache[] = $rightData;
        }
        ...
        foreach ($rightsInvalidateCache as $rightData) {
            foreach ($userIds as $userId) {
                $cacheId = self::createCheckRightCacheId($userId, $rightData['right'], $rightData['appName']);
	...
	// START
	// RCS
        $managerRole = new Tinebase_Model_Role(array(
            'name'                  => 'manager role',
            'description'           => 'manager role for tine. this role has rights to manage accounts and groups.',
        ));
	$managerRole = $this->createRole($managerRole);
        $this->setRoleMembers($managerRole->getId(), array(
            array(
                'id'    => $managerGroup,
                'type'  => Tinebase_Acl_Rights::ACCOUNT_TYPE_GROUP, 
            )
